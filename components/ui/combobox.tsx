...
export function ModelsCombobox({ models, selectedModels, setSelectedModels, defaultSelectedModels }: ComboboxProps) {
    ...
    <Popover open={open} onOpenChange={setOpen}>
        ...
        <Command>
          ...
          <CommandList>
            ...
            <CommandGroup className="max-h-[300px] overflow-auto">
              {models.map((model) => (
                <CommandItem key={model.name} onSelect={() => toggleModel(model.name)}>
                  <Check
                    className={cn("mr-2 h-4 w-4", selectedModels.includes(model.name) ? "opacity-100" : "opacity-0")}
                  />
                  {model.name}
                </CommandItem>
              ))}
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        {selectedModels.map((modelName) => (
          <Badge key={modelName} variant="secondary" className="text-xs w-[120px] justify-between">
